# The script from huggingface's tutorial.
# Source: https://github.com/huggingface/diffusers/blob/main/examples/research_projects/lora/train_text_to_image_lora.py
...
def parse_args():
    ...
    # lora args
    parser.add_argument(
        "--use_peft", action="store_true", help="Whether to use peft to support lora"
    )
    parser.add_argument(
        "--lora_r", type=int, default=4, help="Lora rank, only used if use_lora is True"
    )
    parser.add_argument(
        "--lora_alpha",
        type=int,
...
def main():
    ...
    if accelerator.is_main_process:
        if args.use_peft:
            ...
            lora_config["peft_config"] = unwarpped_unet.get_peft_config_as_dict(
                inference=True
            )
            if args.train_text_encoder:
                ...
                lora_config["text_encoder_peft_config"] = (
                    unwarpped_text_encoder.get_peft_config_as_dict(inference=True)
                )

            accelerator.save(
                state_dict, os.path.join(args.output_dir, f"{global_step}_lora.pt")
            )
            with open(
                os.path.join(args.output_dir, f"{global_step}_lora_config.json"), "w"
            ) as f:
    ...
    if args.use_peft:
        ...
        def load_and_set_lora_ckpt(pipe, ckpt_dir, global_step, device, dtype):
            with open(
                os.path.join(args.output_dir, f"{global_step}_lora_config.json"), "r"
            ) as f:
